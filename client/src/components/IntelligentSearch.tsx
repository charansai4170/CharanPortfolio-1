...
const IntelligentSearch = ({ onSearch, onAddToHistory }: SearchProps) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [isTyping, setIsTyping] = useState(false);
  const [currentSuggestionSet, setCurrentSuggestionSet] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  
  // Contextual suggestions tailored for Charan/NNA
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 p-4 md:p-6 bg-gradient-to-t from-black/50 via-black/20 to-transparent backdrop-blur-sm">
